...
RUN apt-get update -y \
    && apt-get install -y python3 python3-pip git zip bzip2 sudo wget vi daemonize \
...
RUN git clone https://github.com/riverlane/paris ${PARIS}
...
#
# docker build --tag hackathon.20190127 ck-quantum/docker/hackathon.20190127
...
#
CMD daemonize -o ${HOME}/ck_server.out -e ${HOME}/ck_server.err /usr/local/bin/ck display dashboard --scenario=hackathon.20190127 --host=0.0.0.0 --wfe_host=localhost --wfe_port=3355 && /bin/bash
